...
router.post("/getOTP", async (req, res) => {
  ...
  if (rsAddOTP) {
    var mailOptions = {
      ...
      html:
        `<p stlye="color:black">Dear customer. <br></br>` +
        "You are receiving this because you have private service from us.<br></br>" +
        "Please use this OTP to confirm your service: " +
        `<h2>${otpCode}</h2>` +
        "<br></br>" +
  ...
  response(
    res,
    "",
    `Send OTP successful. Check your email: ${req.body.email}`,
    {}
  );
...
router.get("/:role", async (req, res) => {
  ...
  if (role == 1) type = "customer";
  else if (role == 2) type = "employer";
  else type = "admin";
